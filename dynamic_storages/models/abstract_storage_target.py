...
class AbstractStorageTarget(models.Model):
    ...
    last_checked = models.DateTimeField(
        ...
        editable=False,
        help_text=_(
            "Timestamp identifying when this storage provider was last checked"
        ),
    )
    ...
    provider = models.CharField(
        ...
        help_text=_(
            "Specific storage provider this target utilizes - generally an object storage solution of some sort"
        ),
    )
    ...
    def storage_backend(self):
        ...
        if self.provider in [
            k for k, v in STORAGE_PROVIDER_MAP.items() if v.get("class", None)
        ]:
            if not getattr(self, "_storage_backend", None):
                ...
                if self.provider == "gcloud":
                    ...
                    create_kwargs["credentials"] = (
                        service_account.Credentials.from_service_account_info(
                            create_kwargs.pop("credentials", {})
                        )
                    )
                    log.debug(
                        "Storage Target - storage_backend - created credentials object for Google APIs from credential json provided."
                    )
                if self.provider == "do":
                    ...
                    if not create_kwargs.get("region_name"):
                        ...
                        log.debug(
                            "Storage Target - storage_backend - set region_name to SFO1 for DigitalOcean as no other value was set"
                        )
                    create_kwargs["endpoint_url"] = (
                        "https://{}.digitaloceanspaces.com".format(
                            create_kwargs.get("region_name", "SFO1")
                        )
                    )
                elif self.provider == "s3boto3":
                    create_kwargs["default_acl"] = create_kwargs.get(
                        "default_acl", "bucket-owner-full-control"
                    )
                    log.debug(
                        "Storage Target - storage_backend - set default_acl to {}".format(
                            create_kwargs["default_acl"]
                        )
                    )
                self._storage_backend = create_class(**create_kwargs)
                log.debug(
                    "Storage Target - storage_backend - set local _storage_backend value for {}".format(
                        self.provider
                    )
                )
            ...
            log.debug(
                "Storage Target - storage_backend - No storage provider can be determined from this model, returnin the Django `default_storage` backend"
            )
